...
function renderDetail(order, links) {
  ...
  const steps = [
    ...
    ["Certificaat gegenereerd", "Zoho Creator", "Bron niet gekoppeld"], ["E-mail aangeboden voor verzending", "E-mailactie", order.email_submission_status === "submitted" ? "Bewezen door monitoringevent" : order.email_submission_status === "failed" ? "Mislukking geregistreerd" : "Bron niet gekoppeld"]
  ];
...
async function load() {
  ...
  try {
    ...
    summary.innerHTML = [[data.counts.new,"Nieuwe boomverkopen"],[data.counts.completed,"Volledig afgehandeld"],[data.counts.processing,"In verwerking"],[data.counts.unverifiable,"Niet volledig controleerbaar"],[data.counts.action_required,"Actie nodig"]].map(([value,label]) => `<div class="metric"><strong>${value}</strong><span>${label}</span></div>`).join("");
