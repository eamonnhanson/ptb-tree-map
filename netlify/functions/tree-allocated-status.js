...
export function determineTreeAllocatedStatus(order, now = new Date(), windowHours = DEFAULT_PROCESSING_WINDOW_HOURS) {
  ...
  if (order.certificate_sent === false && overdue) reasons.push("Certificaatmail niet verzonden");
  if (order.technical_error) reasons.push(order.technical_error);

  const emailConfirmed = order.email_submitted === true || order.certificate_sent === true;
  const sourcesComplete = ordered !== null && creatorCount !== null && (order.email_submitted !== null || order.certificate_sent !== null);
  const requiredSourceUnavailable = order.shopify_source_available === false ||
    order.creator_source_available === false;
  const complete = order.user_id && ordered === allocated && order.mismatched_order_count === 0 &&
    creatorCount === 1 && emailConfirmed;

  if (complete) return { status: "completed", label: "Afgerond", reasons: [] };
