  ...
  try {
    ...
    const [
      ...
      monitoringPool.query(
        "select count(*)::int as count from monitoring.automation_registry"
      ),
      monitoringPool.query(
        "select count(*)::int as count from monitoring.automation_events"
      ),
      monitoringPool.query(
        "select count(*)::int as count from monitoring.outbound_messages"
      )
    ]);
    ...
    return json(200, {
      ...
      counts: {
        automation_registry: automationRegistryResult.rows[0]?.count ?? 0,
        automation_events: automationEventsResult.rows[0]?.count ?? 0,
        outbound_messages: outboundMessagesResult.rows[0]?.count ?? 0
      },
