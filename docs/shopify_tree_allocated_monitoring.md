...
Tree allocated leest geen Shopify API. Zap `374491281` schrijft drie compacte events via **Webhooks by Zapier → Custom Request** naar `POST https://map.planteenboom.nu/.netlify/functions/monitoring-ingest` en daarmee naar de bestaande tabel `monitoring.automation_events`. Stel bij Custom Request `Method` in op `POST`, kies een JSON-body en stel de headers in op `Content-Type: application/json` en `Authorization: Basic <base64 van AUTOMATION_DASHBOARD_USER:AUTOMATION_DASHBOARD_PASSWORD>`. Gebruik daarvoor de bestaande beveiligde waarden uit Netlify/Zapier; zet het wachtwoord nooit in een payload of logveld.

## Canonieke workflowidentiteit

| Registryveld | Waarde | Verificatiestatus |
|---|---|---|
| `workflow_key` | `shopify_gift_tree_sku01_374491281` | Geverifieerd en strikt gevalideerd. |
| `workflow_name` | `DEV - Shopify → Tokenized Gift Tree Link - Multilingual` | Geverifieerd. |
| `zap_id` | `374491281` | Geverifieerd en strikt gevalideerd. |
| `displayed_published_version` | `v4` | Geverifieerd als zichtbaar label in de Zapier-UI; dit is geen intern ID. |
| `internal_zapier_version_id` | `unverified` | Niet bekend en niet toegestaan in webhookpayloads. |
| `editor_state` | `unpublished draft of Zap 374491281` | De editor bevat een ongepubliceerde draft van dezelfde Zap. |

Vaste velden in elke request zijn `workflow_key=shopify_gift_tree_sku01_374491281` en `zap_id=374491281`. Laat `zap_version` weg totdat een gezaghebbende, door Zapier beheerde bron een intern versie-ID bewijst. Zolang dat ID niet is geverifieerd, weigert de endpoint iedere expliciet meegestuurde `zap_version`, inclusief de eerder veronderstelde waarde `374491282`. De server maakt de idempotency key ongewijzigd uit workflow, eventtype en het genormaliseerde order-ID.
...
Voorbeeldbody (waarden tussen `<...>` worden in Zapier gemapt):

```json
{
  "event_type": "shopify_order_received",
  "order_id": "<Shopify Order ID>",
  "occurred_at": "<Shopify Created At>",
  "workflow_key": "shopify_gift_tree_sku01_374491281",
  "zap_id": "374491281",
  "created_at": "<Shopify Created At>",
  "customer_email": "<Shopify Customer Email>",
  "customer_locale": "<Shopify Customer Locale>",
...
```json
{
  "event_type": "gift_claim_created",
  "order_id": "<dezelfde Shopify Order ID>",
  "occurred_at": "{{zap_meta_human_now}}",
  "workflow_key": "shopify_gift_tree_sku01_374491281",
  "zap_id": "374491281",
  "creator_record_id": "<Creator record-ID uit de geslaagde actie>",
...
```json
{
  "event_type": "gift_claim_email_submitted",
  "order_id": "<dezelfde Shopify Order ID>",
  "occurred_at": "{{zap_meta_human_now}}",
  "workflow_key": "shopify_gift_tree_sku01_374491281",
  "zap_id": "374491281",
  "recipient_email": "<Shopify Customer Email>",
  "language": "<genormaliseerde taal: nl, en of fr>",
  "submission_status": "submitted",
  "provider_or_action": "Zoho Mail - Send claim email"
}
```
