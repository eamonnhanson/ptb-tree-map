-- PRODUCTION CUTOVER EXECUTION - REQUIRES EXPLICIT PRODUCTION APPROVAL.
-- DO NOT RUN BEFORE THE FINAL DELTA CHECK HAS PASSED.
-- Migration 014 must already have been applied successfully. This file does not
-- duplicate or alter migration 014. Any failed assertion aborts this transaction.
...
DECLARE
  ...
  v_welcome_not_required integer;
  v_billing_period_total bigint;
  v_payment_event_total bigint;
  v_trees_total bigint;
BEGIN
  -- Object and policy preflight. These checks precede every write.
  IF to_regclass('public.chargebee_subscription_cutover_policy') IS NULL THEN
    RAISE EXCEPTION 'Migration 014 prerequisite missing: chargebee_subscription_cutover_policy';
  END IF;
  IF to_regclass('public.chargebee_tree_subscriptions') IS NULL THEN
    RAISE EXCEPTION 'Migration 014 prerequisite missing: chargebee_tree_subscriptions';
  END IF;
  IF to_regclass('public.chargebee_subscription_plans') IS NULL THEN
    RAISE EXCEPTION 'Migration 014 prerequisite missing: chargebee_subscription_plans';
  END IF;
  IF to_regclass('public.users1') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: users1';
  END IF;

  SELECT count(*) INTO v_count
  FROM public.chargebee_subscription_cutover_policy;
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'Cutover policy conflict: expected empty policy table, found % rows', v_count;
  END IF;
  ...
  SELECT count(*) INTO v_count
  FROM public.chargebee_subscription_cutover_policy
  WHERE singleton AND cutover_at = v_cutover_at;
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'Post-flight cutover policy mismatch';
  END IF;

  SELECT count(*),
    count(*) FILTER (WHERE subscription.initial_side_effects_suppressed),
    count(*) FILTER (WHERE subscription.welcome_status = 'not_required'),
    coalesce(sum(subscription.successful_billing_period_count), 0),
    coalesce(sum(subscription.successful_payment_event_count), 0),
    coalesce(sum(subscription.trees_allocated_total), 0)
  INTO v_count, v_suppressed, v_welcome_not_required,
       v_billing_period_total, v_payment_event_total, v_trees_total
  FROM public.chargebee_tree_subscriptions AS subscription
  JOIN jsonb_to_recordset(v_seed) AS seed(chargebee_subscription_id text)
    USING (chargebee_subscription_id);

  IF v_count <> v_expected_rows
     OR v_suppressed <> v_expected_rows
     OR v_welcome_not_required <> v_expected_rows
     OR v_billing_period_total <> 0
     OR v_payment_event_total <> 0
     OR v_trees_total <> 0 THEN
    RAISE EXCEPTION
      'Post-flight aggregate mismatch: rows %, suppressed %, welcome %, periods %, events %, trees %',
      v_count, v_suppressed, v_welcome_not_required,
      v_billing_period_total, v_payment_event_total, v_trees_total;
  END IF;
END;
$seed_block$;

-- Post-flight evidence must be exactly:
...
-- Commit only after every preflight, write-count and post-flight assertion passes.
COMMIT;
